...
echo "Start BERT_L Training on ${MPI_NUM_PROCS} card, BS: ${BATCH_SIZE}, epochs: ${EPOCH_NUM}, log: ${MPI_LOG} at $starttime"
...
$MPI_RUN --allow-run-as-root --output-filename $MPI_LOG  \
	...
	--hostfile $HOSTFILE \
        ...
        -x NUM_BUCKET \
        -x MPI_NUM_PROCS $PROJECT_DIR/tests/dist_train_nnp_bert_l_pretrain.sh

endtime=`date +'%Y-%m-%d %H:%M:%S'`

start_seconds=$(date --date="$starttime" +%s)
end_seconds=$(date --date="$endtime" +%s)
echo "Finish BERT_L training task at $endtime, cost $((end_seconds-start_seconds)) sec"
